...
object DatabaseExtractorFactory {
    ...
    fun getExtractor(config: ApplicationConfiguration): DatabaseExtractor {
        return if (config.getDatabaseConfiguration().getType() == DBType.POSTGRESQL) {
            PostgresExtractor(config)
        } else {
            throw DatabaseMigrationFailure("Unsupported database type: " + config.getDatabaseConfiguration().getType())
        }
    }
}