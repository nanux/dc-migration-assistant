...
class S3SyncFileSystemDownloader internal constructor(
    private val ssmApi: SSMApi,
    private val maxCommandStatusRetries: Int
) {
    ...
    fun initiateFileSystemDownload() { // FIXME: Reload the migration stack instance ID in case instance has gone down during migration
        ...
        val consumer: SuccessfulSSMCommandConsumer<*> =
            EnsureSuccessfulSSMCommandConsumer(ssmApi, commandID, MIGRATION_STACK_INSTANCE)
        try {
    ...
    fun getFileSystemDownloadStatus(): S3SyncCommandStatus? {
        ...
        val consumer: SuccessfulSSMCommandConsumer<S3SyncCommandStatus> =
            UnmarshalS3SyncStatusSSMCommandConsumer(ssmApi, statusCommandId, MIGRATION_STACK_INSTANCE)
        return try {
            consumer.handleCommandOutput(maxCommandStatusRetries)
        } catch (e: UnsuccessfulSSMCommandInvocationException) {
    ...
    companion object {
        ...
        private val SSM_PLAYBOOK = System.getProperty(
            "com.atlassian.migration.s3sync.documentName",
            "bpartridge-12-03t15-42-39-migration-helper-SharedHomeDownloadDocument-1C56C88F671YL"
        )

        // FIXME: Should be loaded from migration stack.
        private val STATUS_SSM_PLAYBOOK =
            System.getProperty("com.atlassian.migration.s3sync.statusDocmentName", "fake-document")

        // FIXME: Should be loaded from migration stack. Defaults to an instance deployed by a migration stack in us-east-1
        private val MIGRATION_STACK_INSTANCE =
            System.getProperty("com.atlassian.migration.instanceId", "i-0353cc9a8ad7dafc2")
    }
}